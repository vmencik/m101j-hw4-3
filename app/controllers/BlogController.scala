...
trait BlogControllerApi extends Controller {
      ...
      comment => {
        blogPostDAO.findByPermalink(comment.permalink) match {
          case Some(post) =>
            ...
            } else Ok(views.html.postDetail(currentUser, post, validated)) // bounce this back to the user for correction
          case _ => Ok(views.html.postNotFound())
        }
      })
  }
  ...
  def postsByTag(thetag: String) = Action { implicit request =>
    val posts = blogPostDAO.findByTagDateDescending(thetag)
